...
class LostController {
  ...
  static async resolveLost(req, res) {
    try {
      const { id } = req.params;

      const result = await lostitemService.resolveLostItem(id);
      ...
      console.error("Controller Error resolving item:", error);
      res.status(500).json({
        success: false,
        message: "A technical error occurred while resolving the item.",
      });
    }
  }
